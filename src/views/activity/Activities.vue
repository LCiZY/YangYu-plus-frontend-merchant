          ...
          <div slot="left" class="demo-split-pane" style="height: 100%">
            <Card :bordered="false">
              <a :href="chooseActivity.activityCoverPicUrl" target="_blank">
                <img
                  :src="chooseActivity.activityCoverPicUrl"
                  class="image-size"
                  style="float: right"
                />
              </a>
              ...
              <h3
                v-show="
                  chooseActivity.merchantId && chooseActivity.merchantId !== ''
                "
              >
                创建商家ID：{{ chooseActivity.merchantId }}
              </h3>
              ...
              <h3>活动描述</h3>
              <p>{{ chooseActivity.activityDesc }}</p>
            </Card>
          </div>
            ...
            <Split v-model="splitRight" mode="vertical">
              <div slot="top" class="demo-split-pane">
                <Card :bordered="false" style="height: 100%">
                  <a :href="chooseUserInfo.userAvatarUrl" target="_blank">
                    <img
                      :src="chooseUserInfo.userAvatarUrl"
                      class="image-size"
                      style="float: right"
                      alt="无法显示"
                    />
                  ...
                  <h4>
                    用户性别：{{
                      chooseUserInfo.userGender
                    }}；&nbsp;&nbsp;孩子性别：{{
                      chooseUserInfo.userChildGender
                    }}
                  </h4>
                  <h4>用户孩子所处阶段：{{ chooseUserInfo.userChildStage }}</h4>
                  <h4>用户孩子生日：{{ chooseUserInfo.userChildBirthday }}</h4>
                  <h4>
                    用户上次登陆时间：{{ chooseUserInfo.userLastLoginTime }}
                  </h4>
              ...
              <div slot="bottom" class="demo-split-pane">
                <Card :bordered="false" style="height: 100%">
                  <h2>活动图片</h2>
                  <a
                    v-for="item in chooseActivity.activityImages"
                    :key="item.activityImageId"
                    :href="item.activityImageUrl"
                    target="_blank"
                  >
              ...
              <Form :model="chooseActivity" :label-width="120">
                <Row>
                  ...
                  <Col span="11">
                    <FormItem
                      label="用户ID"
                      v-if="
                        chooseActivity.userId && chooseActivity.userId !== ''
                      "
                    >
                ...
                <Row>
                  ...
                  <Col span="11">
                    <FormItem label="活动类型">
                      <Input
                        v-model="chooseActivity.activityType"
                        placeholder="输入活动类型"
                      />
                    </FormItem>
                  </Col>
                ...
                <Row>
                  <Col span="11">
                    <FormItem label="活动日期">
                      <DatePicker
                        type="date"
                        v-model="activityDateFromServer"
                        format="yyyy-MM-dd"
                        placeholder="选择活动日期"
                ...
                <Row>
                  <Col span="11">
                    <FormItem label="活动地点">
                      <Input
                        v-model="chooseActivity.activityPlace"
                        placeholder="输入活动地点"
                      />
                    </FormItem>
                  </Col>
                  <Col span="11">
                    <FormItem label="活动对象">
                      <Input
                        v-model="chooseActivity.activityObject"
                        placeholder="输入活动对象"
                      />
                    </FormItem>
                  </Col>
                ...
                <Row>
                  <Col span="11">
                    <FormItem label="活动人数">
                      <Input
                        v-model="chooseActivity.activityMembers"
                        placeholder="输入活动人数"
                      />
                    </FormItem>
                  </Col>
                  <Col span="11">
                    <FormItem label="活动平均花费">
                      <Input
                        v-model="chooseActivity.activityAverCost"
                        placeholder="输入活动平均花费"
                      />
                    </FormItem>
                  </Col>
                </Row>
                <FormItem label="活动描述">
                  <Input
                    v-model="chooseActivity.activityDesc"
                    type="textarea"
                    placeholder="输入活动描述"
                  />
                </FormItem>
          ...
          >
            <Card :bordered="false" style="height: 100%">
              ...
              >
                ...
                <div
                  class="demo-upload-list-cover image-size"
                  style="left: 50%; transform: translateX(-50%)"
                >
                  <Icon
                    type="ios-eye-outline"
                    @click.native="handleView('活动封面图', coverPicFileUrl)"
                  ></Icon>
                </div>
                <br />
              </div>
              ...
              <div
                class="demo-upload-list"
                v-for="item in showFilesUrlList"
                :key="item.activityImageUrl"
              >
...
import { homeDataActivityCount } from "../home/Home";
                ...
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      ...
                      this.chooseActivity = params.row;
                      this.activityDateFromServer =
                        this.chooseActivity.activityDate.split(" ")[0];
                      this.activityTime =
                        this.chooseActivity.activityDate.split(" ")[1]
                          ? this.chooseActivity.activityDate.split(" ")[1]
                          : "09:30";
                      this.updateModal = true;
                      this.coverPicFileUrl =
                        this.chooseActivity.activityCoverPicUrl;
                      let i = 0;
                      for (
                        ;
                        i < this.chooseActivity.activityImages.length;
                        i++
                      ) {
                        this.showFilesUrlList.push({
                          name:
                            "image_" +
                            this.chooseActivity.activityImages[i]
                              .activityImageId,
                          activityImageUrl:
                            this.chooseActivity.activityImages[i]
                              .activityImageUrl,
  ...
  methods: {
    ...
    search() {
      ...
      if (this.searchType === "id") {
        ...
        // 根据类型查询queryActivityByType
        get("/activity/queryCompleteActivityByType", {
          params: {
            activityType: this.searchContent,
    ...
    updateActivity() {
      ...
      if (this.chooseActivity.userId) {
        form.append("userId", this.chooseActivity.userId);
      }
      if (this.chooseActivity.merchantId) {
        form.append("merchantId", this.chooseActivity.merchantId);
      }
      ...
      if (this.activityCoverPic) {
        form.append(
          "activityCoverPic",
          this.activityCoverPic,
          this.activityCoverPic.name
        );
      }
      ...
      post("/activity/updateActivity", form, {
    ...
    handleRemove(file) {
      ...
      if (index === -1) {
        ...
        for (; i < this.showFilesUrlList.length; i++) {
          if (
            this.showFilesUrlList[i].activityImageId === file.activityImageId
          ) {

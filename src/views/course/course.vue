...
export default {
              ...
              h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      this.$router.push({
                        path: `updateCourse/${params.row.courseId}`,
                      });
                      instance.reloadPage();
                    },
                  },
  ...
  watch: {
    $route: {
      handler: function (val, oldVal) {
        ...
        if (val.query.courseId) {
          this.searchFormItem.courseId = val.query.courseId;
          this.search();
        }
      },
      deep: true, //true 深度监听
  ...
  mounted() {
    ...
    if (this.$route.query.courseId) {
      this.searchFormItem.courseId = this.$route.query.courseId;
      this.search();
    } else {
  ...
  methods: {
    ...
    search(start, number) {
      ...
      if (this.searchFormItem.courseId && this.searchFormItem.courseId !== "") {
        ...
        if (merchantId == null || !isNumber(merchantId)) {
          return;
        }
        ...
        get("/course4b/queryMerchantsCourses", {
          params,
        })
          .then((res) => {
            if (res && res != "") {
    ...
    queryCoursePageByPage(index) {
      ...
      let merchantId = localStorage.getItem("merchantId");
      if (merchantId == null || !isNumber(merchantId)) return;
      get("/course4b/queryMerchantsCourses", {
        params: {
          start: startIndex,
    ...
    platformActivateCourse() {
      // 上架课程
      get("/course4b/platformActivateCourse", {
    ...
    platformDeactivateCourse() {
      // 下架课程
      get("/course4b/platformDeactivateCourse", {
        params: {

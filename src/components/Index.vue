<template>
  <div class="index-vue">
    ...
    <aside :class="asideClassName">
      <!-- logo -->
      <div class="logo-c">
        <span v-show="isShowAsideTitle">洋芋+商家后台</span>
      </div>
      <!-- 菜单栏 -->
      <Menu
        class="menu"
...
export default {
  ...
  mounted() {
    ...
    // 设置用户信息
    this.userName = localStorage.getItem("merchantName");
  ...
  methods: {
    ...
    closePath(path) {
      ...
      for (let i = 0, len = this.tagsArry.length; i < len; i++) {
        console.log("path", this.tagsArry[i]);
        if (this.tagsArry[i].path && re.test(this.tagsArry[i].path)) {
          this.closeTag(i);
          break;
        }
      }
